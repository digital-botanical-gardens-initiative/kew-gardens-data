...
def merge_excel_files(input_directory, output_file):
    ...
    for i, df in enumerate(dataframes):
        print(
            f"File {i+1}: {os.path.basename(all_files[i])} - Columns: {list(df.columns)}"
        )

    # Check if the files contain the same columns
    if not all(set(df.columns) == set(dataframes[0].columns) for df in dataframes):
        raise ValueError(
            "Not all files have the same columns. Please check the input files."
        )
    ...
    if initial_row_count != final_row_count:
        print(
            f"Removed {initial_row_count - final_row_count} duplicate rows from the merged dataframe."
        )
...
def output_family_barchart(merged_df, output_file):
    ...
    vega_spec = {
        ...
        "height": 200,
        "padding": 5,
        "data": [{"name": "table", "values": family_counts.to_dict(orient="records")}],
        "scales": [
            {
                "name": "xscale",
                "type": "band",
...
if __name__ == "__main__":
    ...
    family_barchart_file = "data/output/kew-family-barchart.vg.json"
